...
func _on_click() -> void:
	if I.is_item_in_inventory("Key"):
		await C.player.walk_to_clicked()
		await C.player.face_clicked()
		await C.player.say("It's empty.")
	else:
		await C.player.walk_to_clicked()
		await C.player.face_clicked()
		await change_frame(1)
		I.Key.add()
		await C.player.say("[shake]Hey, a key![/shake]")
...
func _on_item_used(_item: PopochiuInventoryItem) -> void:
	if _item.script_name == 'Key':
		await C.player.say("I don't want to put it back away yet!")
	else:
		await C.player.say("Nah!")
